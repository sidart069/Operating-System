...
struct proc{
    ...
    int service_time;
    int arrival_time;
    int priority;      //cummulative time for the process to run
   	
};	

bool compP(struct proc p1, struct proc p2){
...
void comparePriority(struct proc p[]){
    sort(p+1,p+4,compP);
...
void findAvgTime(struct proc p[]){
    ...
    cout<<"process "<<" burst time "<<" waiting time "<<" turn around-time"<<"priority "<<"Arrival time"<<"\n";
   
    for(int i=0;i<4;i++){
        total_wt = total_wt + p[i].waitT;
        total_tat = total_tat + p[i].tat;
        cout<<"  "<<i+1<<"\t\t"<<p[i].burstTime<<"\t"<<p[i].waitT<<"\t\t  "<<p[i].tat<<"\t\t  "<<p[i].priority<<"\t\t  "<<p[i].arrival_time<<endl;        
    }
...
int main(){
   proc p[] = {{2,0,0,0,1,3},{6,0,0,0,0,2}, {5,0,0,0,2,1},{4,0,0,0,3,4} };
